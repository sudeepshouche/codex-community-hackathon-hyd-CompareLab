...
DEFAULT_WHISPERX_CPU_COMPUTE_TYPE = os.environ.get(
    "TRIBEV2_WHISPERX_CPU_COMPUTE_TYPE",
    "int8",
)
DEFAULT_WHISPERX_CUDA_COMPUTE_TYPE = os.environ.get(
    "TRIBEV2_WHISPERX_CUDA_COMPUTE_TYPE",
    "float16",
)
DEFAULT_WHISPERX_MODEL = os.environ.get("TRIBEV2_WHISPERX_MODEL", "large-v3")
DEFAULT_WHISPERX_BATCH_SIZE = os.environ.get("TRIBEV2_WHISPERX_BATCH_SIZE", "16")
...
class ExtractWordsFromAudio(EventsTransform):
    ...
    def _get_transcript_from_audio(wav_filename: Path, language: str) -> pd.DataFrame:
        ...
        compute_type = (
            DEFAULT_WHISPERX_CUDA_COMPUTE_TYPE
            if device == "cuda"
            else DEFAULT_WHISPERX_CPU_COMPUTE_TYPE
        )

        with tempfile.TemporaryDirectory() as output_dir:
            logger.info(
                "Running whisperx via uvx device=%s compute_type=%s model=%s batch_size=%s",
                device,
                compute_type,
                DEFAULT_WHISPERX_MODEL,
                DEFAULT_WHISPERX_BATCH_SIZE,
            )
            cmd = [
                "uvx",
                "whisperx",
