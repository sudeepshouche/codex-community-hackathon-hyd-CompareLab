...
DEFAULT_TEXT_WORD_DURATION_SEC = 0.4
TEXT_TOKEN_RE = re.compile(r"\b[\w']+\b")
...
class TextToEvents(pydantic.BaseModel):
    """Convert raw text into lightweight local text events."""
    ...
    def get_events(self) -> pd.DataFrame:
        tokens = TEXT_TOKEN_RE.findall(self.text)
        if not tokens:
            raise ValueError("Text input does not contain any tokens.")
            ...
            events.append(
                {
                    ...
                    "text": token,
                    "context": self.text,
                    "start": round(cursor_s, 3),
                    "duration": DEFAULT_TEXT_WORD_DURATION_SEC,
                    "timeline": timeline,
                    "subject": subject,
                }
            )
            cursor_s += DEFAULT_TEXT_WORD_DURATION_SEC
        ...
        events.append(
            {
                ...
                "text": self.text,
                "start": 0.0,
                "duration": round(max(cursor_s, DEFAULT_TEXT_WORD_DURATION_SEC), 3),
                "timeline": timeline,
                "subject": subject,
            }
        )
...
class TribeModel(TribeExperiment):
    ...
    def get_events_dataframe(
        ...
        *,
        ...
        ----------
        text_path:
            Path to a ``.txt`` file. The text is tokenized locally into
            word-level events.
        audio_path:
